...
func (s *MatchArchiveStore) Get(matchID string) (MatchArchiveEntry, bool) {
	...
	// Match-service writes finished snapshots straight to the backing store,
	// bypassing this process's overlay (the gateway only syncs at creation and
	// join). The overlay can therefore be stale -- a creation-time "active"
	// entry shadows the finished DB row forever -- so when the backend supports
	// queries it, not the overlay, is the source of truth. The overlay remains
	// the fallback for entries the async writer has not flushed yet and for
	// query errors.
...
func (s *MatchArchiveStore) LoadMatch(matchID string) (contracts.MatchState, []contracts.ResolvedEvent, bool) {
	...
	entry, ok := s.entries[matchID]
	// Same freshness rule as Get: with a query-capable backend the DB row wins
