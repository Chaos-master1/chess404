...
A full launch-readiness pass ran against live production: Go build/vet/test in a `golang:1.25` container, a production `pnpm build`/typecheck, a Playwright suite against the live site, an unauthenticated request matrix over every `/api/**` route, and a k6 load test. Fixes for everything below are **committed in-branch and require a deploy before they are live**.
...
- **`/dashboard` shipped publicly** — an engine debug console hardwired to `ws://localhost:8765`, which the site's own CSP blocks. It 404s in production and still works locally.
...
- **The match archive only ever receives a snapshot at match *creation*/join time — never at match end.** `syncMatchSnapshotToPlatformService` is called from bootstrap-with-matchId, private-match create and private-match join, and nowhere else. Even with the 400 fixed, finished results/replays will be stale unless match-service pushes a final snapshot (this is what `replay-worker` and `/api/matches/internal/finished-jobs` exist for, and `replay-worker` is not deployed).
- `/history` reads the *public* archive by default, which excludes vs-computer and direct matches, so a solo player's own games will never appear there even once archiving works. It only queries a player's own archive when a `guestId` is focused from a profile link.
